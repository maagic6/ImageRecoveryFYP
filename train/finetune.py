...
def convert_to_np(image, resolution):
    ...
    if min(width, height) < resolution:
        ...
        new_size = (int(resolution * ratio), resolution) if width > height else (resolution, int(resolution * ratio))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if (
                ...
                #original_image = download_image(random.choice(args.val_image_url))
                edited_images = []
                with torch.autocast(
                    str(accelerator.device),
                    enabled=accelerator.mixed_precision == "fp16",
                ):
                    for x in range(len(args.val_image_url)):
                        #index = random.randrange(len(args.val_image_url))
                        logger.info(
                            f"Running validation... \n Generating image with prompt:"
                            f" {args.validation_prompt[x]}."
                        )
                        original_image = download_image(args.val_image_url[x])
                        edited_images.append(
                            pipeline(
                                args.validation_prompt[x],
